...
class SliderComponent extends HTMLElement {
  ...
  update() {
    ...
    if (this.currentPageElements.length > 0 && this.pageTotalElements.length > 0) {
...
class VariantSelects extends HTMLElement {
  ...
  updateSelectionMetadata({ target }) {
    ...
    if (tagName === 'SELECT' && target.selectedOptions.length) {
      ...
      const optionRow = target.closest('.product-form__input');
      const selectionLabel = optionRow?.querySelector('[data-variant-selection-label]');
      if (selectionLabel && target.selectedOptions[0]) {
        selectionLabel.textContent = target.selectedOptions[0].textContent.replace(/\s+/g, ' ').trim();
      }

      const swatchValue = target.selectedOptions[0].dataset.optionSwatchValue;
      const selectedDropdownSwatchValue = optionRow?.querySelector('[data-selected-value] > .swatch');
